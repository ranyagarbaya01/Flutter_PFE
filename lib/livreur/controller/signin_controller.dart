...
import 'package:pharmacy_app/livreur/service/livreur_service.dart';
...
class SignInController extends GetxController {
  ...
  Future<void> signin(User user, context, String role) async {
    update();
    try {
      isLoading.value = true;
      // Appelez votre API pour vous connecter et obtenir le token
      final token = await _livreurService.signin(user, role);
      print('token : $token');

      if (token == null || token.isEmpty || role != 'livreur') {
